...
setwd("~/Documents/CHUM_git/test_chimeras/")
...
rownames(asv_table) <- str_extract(rownames(asv_table), "\\d{5}_T\\d+")
...
# Metadata handling
{
  ...
  colnames(meta)[4] = "cage"
  metadata$run <- 2
  meta_final <- rbind(meta, metadata)
  meta_final <- meta_final[meta_final$timepoint %in% c("35",'49'),]
}
...
ps <- phyloseq(otu_table(asv_table, taxa_are_rows = FALSE),
               tax_table(taxa), sample_data(meta_final))
...
ps <- prune_samples(sample_data(ps)$run == 1, ps)
# ps <- prune_taxa(colSums(otu_table(ps)) > 0, ps)
...
ps_diet <- merge_phyloseq(ps_t35, ps_t49)
ps_flt_diet <- merge_phyloseq(ps_t35_flt, ps_t49_flt)

# Alpha diveristy
{
  existingDirCheck("../figures/uh/diet/")
  graphs = alphaDiversityTimeSeries2(ps_diet, "../figures/uh/diet/", time = "timepoint", group = "diet", writeData = TRUE)
  
  # Stats
  alpha_d <- read.xlsx("../figures/uh/diet/alpha_diversity/alpha_diversity_data.xlsx")
  alpha_d$diet <- factor(alpha_d$diet, levels = c("50", "500"))
  alpha_d$timepoint <- factor(alpha_d$timepoint, levels = c("35", "49"))
  
  # Chao1
  graphs[[1]]+
    scale_fill_manual(values = c("blue","red"),
                      labels = c("50 ppm","500 ppm"))+
    scale_x_discrete(labels = c("5 weeks", "7 weeks"))+
...
# Beta diversity
{
  ...
  betaDiversityTimepoint2Factors(ps_flt_diet, sample_id = "sample_id", timeVariable = "week",
                                 ...
                                 font = "Arial", path = "../figures/uh/diet/beta_diversity/filtered/", 
...
# Relative abundance analysis: finding differential abundant bugs at the species level, for diet groups only
{
  ...
  pathToSave <- "~/Documents/CHUM_git/figures/uh/relative_abundance_diet/"
