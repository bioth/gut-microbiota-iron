...
plot_microbiota_multiFac_timepoints2 <- function(
    ps_object = ps,
    exp_group = 'group',
    subset_group = NULL,
    timePoints = TRUE,
  ...
  if (mean_group == FALSE) {
    ...
    plots <- df_long %>%
      ...
      lapply(function(sub_df){
        ...
        base_plot <- ggplot(sub_df, aes(x = !!as.name(time_variable), y = value, fill = plot_taxa)) +
          facet_wrap(~ .data[[sample_name2]], ncol = 4, scales = "free_y") +
          geom_bar(stat = "identity", width = 0.85) +
          ylab("Relative abundance (%)\n") +
          ggtitle(unique(sub_df[[exp_group]])) +
          guides(fill = guide_legend(reverse = FALSE, title = sub_level, order = 2)) +
          theme(line = element_line(colour = "black", linewidth = 0.5),
                text = element_text(size = 9),
                panel.grid.major = element_blank(),
          ...
          theme(panel.grid.major = element_blank(),
                ...
                text = element_text(size = text_size),
                legend.text = element_markdown(size = legend_size)) +
          geom_bar(aes(alpha = sub_df[, main_level]), stat = "identity",
                   show.legend = ifelse(showOnlySubLegend, FALSE, TRUE)) +
          scale_alpha_manual(values = rep(1, length(unique(sub_df[, main_level]))),
                             guide = guide_legend(order = 1, override.aes = list(fill = main_level_col)),
                             name = main_level) +
          scale_fill_manual('plot_taxa', values = MyColors2)
    ...
    p <- wrap_plots(plots_shared_legend, ncol = 4, guides = "collect") &
...
# Function that takes a stats excel file generated by the function above and generates
# heatmap for the log2fold values and add significance stars 
logFoldSignificanceHmap <- function(stats, selected_comparisons, main_taxons, sub_taxons,
                                    lvl, main_txn_lvl, sub_txn_lvl, group, displaySignificance = TRUE, sub_hmaps = FALSE, path){
  
  stat_hmap=stats[stats$level==lvl,] # Table with only stats for taxon level of interest
  stat_hmap=stat_hmap[stat_hmap[[sub_txn_lvl]] %in% sub_taxons,] # Keep only taxons displayed on the stackbar
  other_tax=sub_taxons[grepl("Others", x = sub_taxons)] # Isolate "Others" taxons
  
  # Bind "Others" rows to dataframe
  other_rows <- data.frame(matrix(1, nrow = length(other_tax), ncol = ncol(stat_hmap)))
  colnames(other_rows) <- colnames(stat_hmap)
  other_rows[[sub_txn_lvl]] <- other_tax
  for(i in 1:length(other_tax)){

    other_rows[[main_txn_lvl]][i] <- ifelse(other_rows[[sub_txn_lvl]][i] == "Others ", "Others", gsub(pattern = "Others ", replacement = "", other_rows[[sub_txn_lvl]][i]))
  }
  other_rows$log2FoldChange <- 0
  
  # For each comparison, rbind the rows
  for(cmp in selected_comparisons){
    other_rows$comparaison <- cmp
    stat_hmap <- rbind(stat_hmap, other_rows)
  }
  ...
  # colnames(stat_hmap)[2] <- txn_lvl
  stat_hmap$comparaison <- factor(stat_hmap$comparaison, levels = selected_comparisons)
  stat_hmap[[sub_txn_lvl]] <- factor(stat_hmap[[sub_txn_lvl]], levels = levels(sub_taxons))
  stat_hmap[[main_txn_lvl]] <- factor(stat_hmap[[main_txn_lvl]], levels = levels(main_taxons))

  # Define color breaks
  stat_hmap$significance <- cut(stat_hmap$padj,
                                breaks = c(-Inf, 0.001, 0.01, 0.05, Inf),
                                labels = c("***", "**", "*", ""),
                                right = FALSE)
  ...
  # for(i in 1:nrow(stat_hmap)){
  #   matched_value <- stats$log2FoldChange[stats[[txn_lvl]] == stat_hmap[[txn_lvl]][i] & stats$comparaison == stat_hmap$comparaison[i]]
  #   stat_hmap$log2FoldChange[i] <- ifelse(length(matched_value) > 0, matched_value, 0)    
  # }
  # 
  
  if(sub_hmaps){
    
    # Loop through main taxons and do a subheatmap for each
    for(main_tax in main_taxons){
      
      sub_df <- stat_hmap[stat_hmap[[main_txn_lvl]] == main_tax,]
      
      # Plot heatmap with ggplot2
      p <- ggplot(sub_df, aes(y = .data[[sub_txn_lvl]], x = comparaison, fill = log2FoldChange)) +
      ...
      if(displaySignificance){
        p <- p + geom_text(aes(label = significance)) # Show significance labels
      }
      
      ggsave(filename = paste0(path, paste0(main_tax, "heatmap.png")), plot = p, bg = "white", height = 5, width = 5, dpi = 300)
...
function (ps_object = ps, exp_group = "group", subset_group = NULL, 
          ...
          minmu = if (fitType == "glmGamPoi") 1e-06 else 0.5, parallel = FALSE) 
{
  ...
  if (mean_group == F) {
    ...
    p <- ggplot(df_long, aes(x = !!sym(sample_name), y = value, fill = plot_taxa)) +
      geom_bar(stat = "identity", width = 0.85) +
      geom_bar(aes(alpha = !!sym(main_level)), stat = "identity", show.legend = TRUE) +
      
      facet_wrap(as.formula(paste("~", exp_group)), scales = "free_x", nrow = n_row, ncol = n_col) +
